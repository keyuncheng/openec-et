    ...
    for (; count < blocksToProcess && priority < LEVEL; priority++) {
      ...
      if (priority == QUEUE_WITH_CORRUPT_BLOCKS) {
        ...
        if (!useOEC) {
          ...
          for (int j=0; j<blocksToProcess && i.hasNext(); j++) {
            BlockInfo item = i.next();
            long blkid = item.getBlockId();
            if (this.oecCorruptList.contains(item) || this.oecRepairList.contains(item)) continue;
            else this.oecCorruptList.add(item);
          }
          continue;
        }
      }
