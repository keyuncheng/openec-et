...
HHXOR::HHXOR(int n, int k, int w, int opt, vector<string> param) {
    ...
    num_symbols = vs_id; // total number of symbols
...
ECDAG* HHXOR::DecodeSingle(vector<int> from, vector<int> to) {
    ...
    for (auto const &pid_group : _pid_group_map) {
        ...
        // ecdag->BindY(_pid_group_code_map[parity_idx], _code_layout[1][parity_idx]);
...
ECDAG* HHXOR::DecodeMultiple(vector<int> from, vector<int> to) {
    ...
    for (int i = 0; i < num_avail_parity; i++) {
        ...
        if (_pid_group_map.find(parity_idx) != _pid_group_map.end()) {
            ...
            // // BindY (sp0)
            // ecdag->BindY(_uncoupled_code_layout[1][parity_idx], _code_layout[1][parity_idx]);
        }
    }
    ...
    //     ecdag->BindY(vidx, cidx_sp0[_k - 1]);
    // } else {
    //     ecdag->BindY(bindx_sp0[0], cidx_sp0[_k - 1]);
    // }
    ...
    //     ecdag->BindY(vidx, cidx_sp1[_k - 1]);
    // } else {
    //     ecdag->BindY(bindx_sp1[0], cidx_sp1[_k - 1]);
    // }
